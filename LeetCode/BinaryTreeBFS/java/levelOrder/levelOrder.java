package LeetCode.BinaryTreeBFS.java.levelOrder;
...
record levelOrderRecord(String[] root, List<List<Integer>> expected) {
}
...
class levelOrder {
    ...
    public static void main(String[] args) {
        levelOrderRecord[] records = new levelOrderRecord[] {
                ...
                new levelOrderRecord(new String[] {}, List.of()),
        };

        int i = 1;
        for (levelOrderRecord record : records) {
            System.out.println("Test Case " + i++);
            levelOrder.testSolution(record);
        }
    }

    private static void testSolution(levelOrderRecord input) {
        // Create the binary tree from the input array using BFS
        TreeNode rootBTree = levelOrder.BFSCreateBinaryTree(input.root());

        // Print the binary tree and the expected output
        System.out.println("Input: root: " + levelOrder.BFSBinaryTreeToStr(rootBTree));
        System.out.println("Expected: " + input.expected());

        // Call the function to be tested
        List<List<Integer>> res = new levelOrder().levelOrderSolution(rootBTree);
