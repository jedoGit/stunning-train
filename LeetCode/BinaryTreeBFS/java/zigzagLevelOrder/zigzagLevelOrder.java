package LeetCode.BinaryTreeBFS.java.zigzagLevelOrder;
...
record zigzagLevelOrderRecord(String[] root, List<List<Integer>> expected) {
}
...
class zigzagLevelOrder {
    public List<List<Integer>> zigzagLevelOrderSolution(TreeNode root) {
        ...
        while (!q.isEmpty()) {
            ...
            // reverse the level node values if the res.length is odd. res.length refers to
            // the last node values added to res.
            if (res.size() % 2 > 0) {
    ...
    public static void main(String[] args) {
        zigzagLevelOrderRecord[] records = new zigzagLevelOrderRecord[] {
                new zigzagLevelOrderRecord(
                        new String[] { "3", "9", "20", "null", "null", "15", "7" },
                        List.of(List.of(3), List.of(20, 9), List.of(15, 7))),
                new zigzagLevelOrderRecord(
                        new String[] { "1" }, List.of(List.of(1))),
                new zigzagLevelOrderRecord(new String[] {}, List.of()),
        };

        int i = 1;
        for (zigzagLevelOrderRecord record : records) {
            System.out.println("Test Case " + i++);
            zigzagLevelOrder.testSolution(record);
        }
    }

    private static void testSolution(zigzagLevelOrderRecord input) {
        // Create the binary tree from the input array using BFS
        TreeNode rootBTree = zigzagLevelOrder.BFSCreateBinaryTree(input.root());

        // Print the binary tree and the expected output
        System.out.println("Input: root: " + zigzagLevelOrder.BFSBinaryTreeToStr(rootBTree));
        System.out.println("Expected: " + input.expected());

        // Call the function to be tested
        List<List<Integer>> res = new zigzagLevelOrder().zigzagLevelOrderSolution(rootBTree);
