...
class Solution:
    ...
    def testSolution(input: Dict[str, List[str|int]]) -> None:
        ...
        print("Input:\n  root: " + Solution.BFSBinaryTreeToStr(binTree))
        print("  p: " + str(input["p"]))
        print("  q: " + str(input["q"]))
        print("Expected: " + str(input["expected"]))

        # Call the function to be tested
        res = Solution().lowestCommonAncestor(binTree, TreeNode(input["p"]), TreeNode(input["q"]))
   
        # Print the result
        print("Result: " + str(res.val))

        # Validate the result
        print("PASS" if res.val == input["expected"] else "FAIL")
        print("-" * 50)
...
if __name__ == "__main__":
    inputArr = [
        {"root": [3,5,1,6,2,0,8,"null","null",7,4], "p": 5, "q": 1, "expected": 3},
        {"root": [3,5,1,6,2,0,8,"null","null",7,4], "p": 5, "q": 4, "expected": 5},
        {"root": [1,2], "p": 1, "q": 2, "expected": 1}]
