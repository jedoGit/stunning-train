...
function testSolution(record) {
  ...
  const result = solution.letterCombinations(record.digits);
  const pass = JSON.stringify(result) === JSON.stringify(record.expected);

  console.log(`Input: digits = "${record.digits}"`);
...
const records = [
  new LetterCombinationsRecord("23", [
    "ad",
    "ae",
    "af",
    "bd",
  ...
  new LetterCombinationsRecord("", []),
  new LetterCombinationsRecord("2", ["a", "b", "c"]),
  new LetterCombinationsRecord("79", [
    "pw",
    "px",
    "py",
    "pz",
